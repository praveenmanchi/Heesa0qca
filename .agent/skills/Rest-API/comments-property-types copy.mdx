...
slug: /figma/rest-api/comments-property-types
---
...
```
+----------------------------------------+^node_offset origin                      ||                                        || +--------------------+                 || |    region_width    |                 || |                    |                 || |region_height       |                 || |                    |                 || |                    |                 || +--------------------+ <- node_offset  |+----------------------------------------+
```
...
### FrameOffset[​](#frameoffset-type "Direct link to FrameOffset")
...
`node_offset`[Vector](/docs/rest-api/file-property-types/#vector-type)

2d vector offset within the frame from the top-left corner.

### Region[​](#region-type "Direct link to Region")
...
`comment_pin_corner`Stringdefault: bottom-right
...
### FrameOffsetRegion[​](#frameoffsetregion-type "Direct link to FrameOffsetRegion")
