---
title: Property types | Developer Docs
slug: /figma/rest-api/file-property-types
---

On this page

Some of the properties mentioned in the [Node types](/docs/rest-api/file-node-types/) section are custom Figma types. We've outlined the specifics of each of these property types below.

Property type

Description

### Color[​](#color-type "Direct link to Color")
...
### RGBColor[​](#rgbcolor-type "Direct link to RGBColor")
...
### ExportSetting[​](#exportsetting-type "Direct link to ExportSetting")
...
### Constraint[​](#constraint-type "Direct link to Constraint")
...
### Rectangle[​](#rectangle-type "Direct link to Rectangle")
...
### ArcData[​](#arcdata-type "Direct link to ArcData")
...
### BlendMode[​](#blendmode-type "Direct link to BlendMode")
...
### MaskType[​](#masktype-type "Direct link to MaskType")
...
### EasingType[​](#easingtype-type "Direct link to EasingType")
...
### FlowStartingPoint[​](#flowstartingpoint-type "Direct link to FlowStartingPoint")
...
### LayoutConstraint[​](#layoutconstraint-type "Direct link to LayoutConstraint")
...
### LayoutGrid[​](#layoutgrid-type "Direct link to LayoutGrid")
...
`boundVariables`Map<String, [VariableAlias](#variablealias-type)\>

A mapping of field to the [VariableAlias](#variablealias-type) of the bound variable.

### Effect[​](#effect-type "Direct link to Effect")
...
`blendMode`[BlendMode](#blendmode-type)
...
`startRadius`Numberbeta

Radius of the starting point of the progressive blur.

`startOffset`[Vector](#vector-type)beta
...
`endOffset`[Vector](#vector-type)beta
...
`offset`[Vector](#vector-type)

How far the shadow is projected in the x and y directions.

`spread`Numberdefault: 0
...
`noiseSize`Numberbeta

Size of the noise effect.

`noiseType`Stringbeta
...
`secondaryColor`[Color](#color-type)beta
...
`clipToShape`Booleanbeta

Whether the texture is clipped to the shape.

### Hyperlink[​](#hyperlink-type "Direct link to Hyperlink")
...
### DocumentationLink[​](#documentationlink-type "Direct link to DocumentationLink")
...
### Paint[​](#paint-type "Direct link to Paint")
...
`gradientHandlePositions`[Vector](#vector-type)\[\]
...
![Linear gradient in a square. Where the gradient is lightest at the top and becomes darker as it moves downwards. First position is at the top edge of the square, in the middle of the edge and the third position is at the top left corner of the square.](/img/rest-api/linear.png)![Angular gradient in a square. Where there is an anchor at the centre of the square and the gradient color changes as it circles around the anchor at the centre. The darkest and lightest parts of the gradient are adjacent. The first position is the anchor at the centre of the square, the second position is to the right of the first position, at the middle of the right edge of the square, the third position is below the first position, at the middle of the bottom edge of the square.](/img/rest-api/angular.png)![Diamond gradient in a square. Where the lightest parts of the gradient forms a diamond shape at the centre of the square. The positions are set exactly same as the angular gradient.](/img/rest-api/diamond.png)![Radial gradient in a square. Where the lightest parts of the gradient forms a flat oval in the square. The first position is at the centre of the square, the second position is to the right of the first position at the centre of the right edge of the square, the third position is between the first position and the bottom edge of the square.](/img/rest-api/radial.png)

`gradientStops`[ColorStop](#colorstop-type)\[\]
...
`imageTransform`[Transform](#transform-type)
...
A reference to an image embedded in this node. To download the image using this reference, use the [GET file images](/docs/rest-api/file-endpoints/#get-images-endpoint) endpoint to retrieve the mapping from image references to image URLs.

`filters`[ImageFilters](#imagefilters-type)default: {}
...
A reference to an animated GIF embedded in this node. To download the image using this reference, use the [GET file images](/docs/rest-api/file-endpoints/#get-images-endpoint) endpoint to retrieve the mapping from image references to image URLs.

* * *

For pattern paints:

`sourceNodeId`Stringbeta
...
`scalingFactor`Numberbeta

Scaling factor of the pattern.

`spacing`[Vector](#vector-type)beta

Spacing of the pattern.

`horizontalAlignment`Stringbeta
...
### Path[​](#path-type "Direct link to Path")
...
### Vector[​](#vector-type "Direct link to Vector")
...
### Size[​](#size-type "Direct link to Size")
...
### Transform[​](#transform-type "Direct link to Transform")
...
### ImageFilters[​](#imagefilters-type "Direct link to ImageFilters")

Defines the image filters applied to an image paint. All values are from -1 to 1.

`exposure`Numberdefault: 0

`contrast`Numberdefault: 0

`saturation`Numberdefault: 0

`temperature`Numberdefault: 0

`tint`Numberdefault: 0

`highlights`Numberdefault: 0

`shadows`Numberdefault: 0

### ColorStop[​](#colorstop-type "Direct link to ColorStop")
...
### PaintOverride[​](#paintoverride-type "Direct link to PaintOverride")

Paint metadata to override default paints.

`fills`[Paint](#paint-type)\[\]
...
`paragraphSpacing`Numberdefault: 0

Space between paragraphs in px.

`paragraphIndent`Numberdefault: 0

Paragraph indentation in px.

`listSpacing`Numberdefault: 0
...
`textCase`Stringdefault: ORIGINAL
...
`textDecoration`Stringdefault: NONE
...
`textAutoResize`Stringdefault: NONE
...
`textTruncation`Stringdefault: DISABLED
...
`hyperlink`[Hyperlink](#hyperlink-type)

Link to a URL or frame.

`openTypeFlags`Map<String, Number>default: {}
...
`lineHeightPercent`Numberdeprecateddefault: 100
...
### TextPathTypeStyle[​](#textpathtypestyle-type "Direct link to TextPathTypeStyle")

\[Available in beta, API is subject to change\] Metadata for character formatting for text on a path.

Has all the properties of [TypeStyle](#typestyle-type) except: `paragraphSpacing`, `paragraphIndent`, `listSpacing`, `textDecoration`, `textAutoResize`, `textTruncation`, `maxLines`, `lineHeightPx`, `lineHeightPercent`, `lineHeightPercentFontSize`, and `lineHeightUnit`.

### Component[​](#component-type "Direct link to Component")
...
`documentationLinks`[DocumentationLink](#documentationlink-type)\[\]
...
### ComponentSet[​](#componentset-type "Direct link to ComponentSet")
...
### Style[​](#style-type "Direct link to Style")
...
### ShapeType[​](#shapetype-type "Direct link to ShapeType")
...
### ConnectorEndpoint[​](#connectorendpoint-type "Direct link to ConnectorEndpoint")
...
`magnet`[ConnectorMagnet](#connectormagnet-type)
...
### ConnectorLineType[​](#connectorlinetype-type "Direct link to ConnectorLineType")
...
### ConnectorTextBackground[​](#connectortextbackground-type "Direct link to ConnectorTextBackground")
...
### ComponentPropertyDefinition[​](#componentpropertydefinition-type "Direct link to ComponentPropertyDefinition")

`type`[ComponentPropertyType](#componentpropertytype-type)
...
`preferredValues?`[InstanceSwapPreferredValue](#instanceswappreferredvalue-type)\[\]

List of user-defined preferred values for this property. Only exists on INSTANCE\_SWAP properties.

### ComponentProperty[​](#componentproperty-type "Direct link to ComponentProperty")
...
### ComponentPropertyType[​](#componentpropertytype-type "Direct link to ComponentPropertyType")
...
### InstanceSwapPreferredValue[​](#instanceswappreferredvalue-type "Direct link to InstanceSwapPreferredValue")
...
### PrototypeDevice[​](#prototypedevice-type "Direct link to PrototypeDevice")
...
### Annotation[​](#annotation-type "Direct link to Annotation")

A note and pinned properties left on a node in Dev Mode.

`label`String

`properties`[AnnotationProperty](#annotationproperty-type)\[\]

### AnnotationProperty[​](#annotationproperty-type "Direct link to AnnotationProperty")
...
### Measurement[​](#measurement-type "Direct link to Measurement")

A pinned distance between two nodes in Dev Mode.

`id`String

`start`[MeasurementStartEnd](#measurementstartend-type)
...
### MeasurementStartEnd[​](#measurementstartend-type "Direct link to MeasurementStartEnd")
...
### MeasurementOffsetInner[​](#measurementoffsetinner-type "Direct link to MeasurementOffsetInner")
...
### MeasurementOffsetOuter[​](#measurementoffsetouter-type "Direct link to MeasurementOffsetOuter")
...
### StrokeWeights[​](#strokeweights-type "Direct link to StrokeWeights")
...
### Overrides[​](#overrides-type "Direct link to Overrides")
...
### VariableAlias[​](#variablealias-type "Direct link to VariableAlias")
...
The id of the variable that the current variable is aliased to. This variable can be a local or remote variable, and both can be retrieved via the [Variables endpoints](/docs/rest-api/file-endpoints/#get-variables-local-endpoint).

### DevStatus[​](#devstatus-type "Direct link to DevStatus")
...
### Interaction[​](#interaction-type "Direct link to Interaction")
...
### Trigger[​](#trigger-type "Direct link to Trigger")
...
Whether this is a [deprecated version](https://help.figma.com/hc/articles/360040035834-Prototype-triggers#h_01HHN04REHJNP168R26P1CMP0A) of the trigger that was left unchanged for backwards compatibility. If not present, the trigger is the latest version.
...
### Action[​](#action-type "Direct link to Action")
...
### BackAction[​](#backaction-type "Direct link to BackAction")
...
### CloseAction[​](#closeaction-type "Direct link to CloseAction")
...
### OpenURLAction[​](#openurlaction-type "Direct link to OpenURLAction")
...
### UpdateMediaRuntimeAction[​](#updatemediaruntimeaction-type "Direct link to UpdateMediaRuntimeAction")
...
### NodeAction[​](#nodeaction-type "Direct link to NodeAction")
...
`navigation`[Navigation](#navigation-type)

`transition`[SimpleTransition](#simpletransition-type) | [DirectionTransition](#directiontransition-type) | null
...
`overlayRelativePosition`[Vector](#vector-type)
...
### Navigation[​](#navigation-type "Direct link to Navigation")
...
### SimpleTransition[​](#simpletransition-type "Direct link to SimpleTransition")
...
### DirectionTransition[​](#directiontransition-type "Direct link to DirectionTransition")
...
### Easing[​](#easing-type "Direct link to Easing")
...
`easingFunctionCubicBezier`[EasingFunctionCubicBezier](#easingfunctioncubicbezier-type)

A cubic bezier curve that defines the easing.

`easingFunctionSpring`[EasingFunctionSpring](#easingfunctionspring-type)
...
### EasingFunctionSpring[​](#easingfunctionspring-type "Direct link to EasingFunctionSpring")
...
### SetVariableAction[​](#setvariableaction-type "Direct link to SetVariableAction")
...
`variableValue`[VariableData](#variabledata-type)

### SetVariableModeAction[​](#setvariablemodeaction-type "Direct link to SetVariableModeAction")
...
### ConditionalAction[​](#conditionalaction-type "Direct link to ConditionalAction")
...
`conditionalBlocks`[ConditionalBlock](#conditionalblock-type)\[\]

### VariableData[​](#variabledata-type "Direct link to VariableData")

A value to set a variable to during prototyping.

`type`[VariableDataType](#variabledatatype-type)

`resolvedType`[VariableResolvedDataType](#variableresolveddatatype-type)

`value`Boolean | Number | String | [Color](#color-type) | [RGBColor](#rgbcolor-type) | [VariableAlias](#variablealias-type) | [Expression](#expression-type)

### VariableDataType[​](#variabledatatype-type "Direct link to VariableDataType")
...
### VariableResolvedDataType[​](#variableresolveddatatype-type "Direct link to VariableResolvedDataType")
...
### Expression[​](#expression-type "Direct link to Expression")

Defines the [Expression](https://help.figma.com/hc/articles/15253194385943) object, which contains a list of [VariableData](#variabledata-type) objects strung together by operators.

`expressionFunction`[ExpressionFunction](#expressionfunction-type)

`expressionArguments`[VariableData](#variabledata-type)\[\]

### ExpressionFunction[​](#expressionfunction-type "Direct link to ExpressionFunction")
...
### ConditionalBlock[​](#conditionalblock-type "Direct link to ConditionalBlock")
...
`condition`[VariableData](#variabledata-type)

`actions`[Action](#action-type)\[\]

### TransformModifier[​](#transformmodifier-type "Direct link to TransformModifier")

Data describing the transform applied to nodes within a [TRANSFORM\_GROUP](/docs/rest-api/file-node-types/#transform-group-props) node.

`type`REPEAT

The type of transform modifier. Currently, the only supported type is REPEAT, which creates repeated copies of the child node.

`unitType`"PIXELS" | "RELATIVE"

The unit used for the offset value of the transform modifier. RELATIVE is a percentage of the child node size.

`offset`Number

The offset value for the transform modifier, in the specified unitType.

`count`Number

The number of times to repeat the child node (REPEAT type only).

`repeatType`"LINEAR" | "RADIAL"

The type of repeat to apply (REPEAT type only).

`axis`"HORIZONTAL" | "VERTICAL"

The axis along which to repeat the child node (LINEAR REPEAT type only).

### ComplexStrokeProperties[​](#complexstrokeproperties-type "Direct link to ComplexStrokeProperties")

Properties for brush and dynamic strokes.

[BasicStrokeProperties](#basicstrokeproperties-type) | [BrushStrokeProperties](#brushstrokeproperties-type) | [DynamicStrokeProperties](#dynamicstrokeproperties-type)

### BasicStrokeProperties[​](#basicstrokeproperties-type "Direct link to BasicStrokeProperties")

Properties for basic strokes.

`strokeType`"BASIC"

The type of stroke.

### DynamicStrokeProperties[​](#dynamicstrokeproperties-type "Direct link to DynamicStrokeProperties")

Properties for dynamic strokes.
...
Frequency of the dynamic stroke pattern.

`wiggle`Number

Wiggle amount of the dynamic stroke pattern.

`smoothen`Number

Smoothness of the dynamic stroke pattern.

### BrushStrokeProperties[​](#brushstrokeproperties-type "Direct link to BrushStrokeProperties")

Properties for brush strokes.

[ScatterBrushStrokeProperties](#scatterbrushstrokeproperties-type) | [StretchBrushStrokeProperties](#stretchbrushstrokeproperties-type)

### ScatterBrushStrokeProperties[​](#scatterbrushstrokeproperties-type "Direct link to ScatterBrushStrokeProperties")

Properties for scatter brush strokes.
...
The node ID of the brush node used for the scatter brush stroke.

`brushName`String

The name of the brush used for the scatter brush stroke.

`gap`Number

The gap between brush instances along the stroke.

`wiggle`Number

The wiggle amount for brush instances along the stroke.

`rotation`Number

The rotation applied to brush instances along the stroke.

`sizeJitter`Number

The size jitter applied to brush instances along the stroke.

`angularJitter`Number

The angular jitter applied to brush instances along the stroke.

### StretchBrushStrokeProperties[​](#stretchbrushstrokeproperties-type "Direct link to StretchBrushStrokeProperties")

Properties for stretch brush strokes.
...
The node ID of the brush node used for the stretch brush stroke.

`brushName`String

The name of the brush used for the stretch brush stroke.

`direction`"FORWARD" | "REVERSE"

The direction in which to stretch the brush along the stroke.

### VariableWidthPoint[​](#variablewidthpoint-type "Direct link to VariableWidthPoint")

Data describing the width of a certain point in a variable width stroke.

`segment`Number

The index of the segment that the variable width point is on.

`position`Number

The position of the variable width point along the segment, between 0 (start) and 1 (end). Note that this parameter represents the Bezier curve parameterization, so for curved segments the position may not align with the visual position of the point -- a position of 0.5 may not be the midpoint of the segment's arc length.

`width`Number

The width multiplier at this point. The final width at this point is 2 \* strokeWeight \* this value.

### TextPathStartData[​](#textpathstartdata-type "Direct link to TextPathStartData")

Data describing the starting position of text in [TEXT\_PATH](/docs/rest-api/file-node-types/#text-path-props) nodes.

`segment`Number

The index of the segment that the text path start is on.

`position`Number

The position along the segment where the text begins. 0 is the start of the segment, 1 is the end.
