---
title: Node types | Developer Docs
...
### DOCUMENT[​](#document-props "Direct link to DOCUMENT")

`children`[Node](/docs/rest-api/files/#node-type)\[\]

An array of canvases attached to the document.

### CANVAS[​](#canvas-props "Direct link to CANVAS")

`children`[Node](/docs/rest-api/files/#node-type)\[\]

An array of top level layers on the canvas.

`backgroundColor`[Color](/docs/rest-api/file-property-types/#color-type)

Background color of the canvas.

`prototypeStartNodeID`Stringdeprecated
...
`flowStartingPoints`[FlowStartingPoint](/docs/rest-api/file-property-types/#flowstartingpoint-type)\[\]default: \[\]

A array of flow starting points sorted by its position in the prototype settings panel.

`prototypeDevice`[PrototypeDevice](/docs/rest-api/file-property-types/#prototypedevice-type)

The device used to view a prototype.

`exportSettings`[ExportSettings](/docs/rest-api/file-property-types/#exportsettings-type)default: \[\]

Export settings for the canvas.

`measurements`[Measurement](/docs/rest-api/file-property-types/#measurement-type)\[\]default: \[\]

An array of measurements displaying pinned distances between nodes on the canvas in Dev Mode.

### FRAME[​](#frame-props "Direct link to FRAME")

`children`[Node](/docs/rest-api/files/#node-type)\[\]

An array of nodes that are direct children of this node.

`locked`Booleandefault: false

If true, layer is locked and cannot be edited.

`background`[Paint](/docs/rest-api/file-property-types/#paint-type)\[\]deprecated

Background of the node. This is deprecated, as backgrounds for frames are now in the `fills` field.

`backgroundColor`[Color](/docs/rest-api/file-property-types/#color-type)deprecated
...
`fills`[Paint](/docs/rest-api/file-property-types/#paint-type)\[\]default: \[\]

An array of fill paints applied to the node.

`strokes`[Paint](/docs/rest-api/file-property-types/#paint-type)\[\]default: \[\]

An array of stroke paints applied to the node.

`complexStrokeProperties`[ComplexStrokeProperties](/docs/rest-api/file-property-types/#complexstrokeproperties-type)default: {}

cspdesc

`variableWidthPoints`[VariableWidthPoint](/docs/rest-api/file-property-types/#variablewidthpoint-type)\[\]default: \[\]

A list of points describing the width profile of variable width strokes.
...
`constraints`[LayoutConstraint](/docs/rest-api/file-property-types/#layoutconstraint-type)
...
`interactions`[Interaction](/docs/rest-api/file-property-types/#interaction-type)\[\]
...
`transitionNodeID`Stringdefault: null

Node ID of node to transition to in prototyping.

`transitionDuration`Numberdefault: null

The duration of the prototyping transition on this node (in milliseconds).

`transitionEasing`[EasingType](/docs/rest-api/file-property-types/#easingtype-type)default: null
...
`absoluteBoundingBox`[Rectangle](/docs/rest-api/file-property-types/#rectangle-type)

Bounding box of the node in absolute space coordinates.

`absoluteRenderBounds`0
...
`size`[Vector](/docs/rest-api/file-property-types/#vector-type)

Width and height of element. This is different from the width and height of the bounding box in that the absolute bounding box represents the element after scaling and rotation. Only present if `geometry=paths` is passed.

`minWidth`Number | nulldefault: null

The minWidth of the frame, or null if not set.

`maxWidth`Number | nulldefault: null

The maxWidth of the frame, or null if not set.

`minHeight`Number | nulldefault: null

The minHeight of the frame, or null if not set.

`maxHeight`Number | nulldefault: null

The maxHeight of the frame, or null if not set.

`relativeTransform`[Transform](/docs/rest-api/file-property-types/#transform-type)
...
`primaryAxisSizingMode`Stringdefault: AUTO
...
`counterAxisSizingMode`Stringdefault: AUTO
...
`primaryAxisAlignItems`Stringdefault: MIN
...
`counterAxisAlignItems`Stringdefault: MIN
...
`counterAxisAlignContent`Stringdefault: AUTO
...
`gridChildHorizontalAlign`Stringdefault: AUTO
...
`gridChildVerticalAlign`Stringdefault: AUTO
...
`gridColumnSpan`Numberdefault: 1
...
`gridColumnAnchorIndex`Numberdefault: 0
...
`gridRowAnchorIndex`Numberdefault: 0
...
`horizontalPadding`Numberdeprecateddefault: 0
...
`verticalPadding`Numberdeprecateddefault: 0
...
`itemSpacing`Numberdefault: 0

The distance between children of the frame. Can be negative. This property is only applicable for auto-layout frames.

`counterAxisSpacing`Numberdefault: 0
...
`layoutPositioning`Stringdefault: AUTO
...
`itemReverseZIndex`Booleandefault: false
...
`strokesIncludedInLayout`Booleandefault: false
...
`layoutGrids`[LayoutGrid](/docs/rest-api/file-property-types/#layoutgrid-type)\[\]default: \[\]
...
`overflowDirection`Stringdefault: NONE
...
`effects`[Effect](/docs/rest-api/file-property-types/#effect-type)\[\]default: \[\]

An array of effects attached to this node (see effects section for more details).

`isMask`Booleandefault: false

Does this node mask sibling nodes in front of it?

`isMaskOutline`Booleandeprecateddefault: false
...
A mapping of a style\_type to style ID (see [Style](/docs/rest-api/file-property-types/#style-type) for more details) of styles present on this node. The style ID can be used to look up more information about the style in the top-level styles field.

`devStatus`0default: null

Whether the node is marked `Ready for dev` or `Completed`. If the node does not have a status, the property is `null`.

`annotations`[Annotation](/docs/rest-api/file-property-types/#annotation-type)\[\]private betadefault: \[\]
...
### GROUP[​](#group-props "Direct link to GROUP")

See properties for [FRAME](#frame-props).

### TRANSFORM\_GROUP[​](#transform-group-props "Direct link to TRANSFORM_GROUP")

\[Available in beta, API is subject to change\] A group that represents a node transform.

Transform groups have all of the properties of a [FRAME](#frame-props), plus:

`transformModifiers`[TransformModifier](/docs/rest-api/file-property-types/#transformmodifier-type)\[\]default: \[\]

### SECTION[​](#section-props "Direct link to SECTION")

`sectionContentsHidden`Booleandefault: false
...
`complexStrokeProperties`[ComplexStrokeProperties](/docs/rest-api/file-property-types/#complexstrokeproperties-type)default: {"type": "BASIC"}
...
### VECTOR[​](#vector-props "Direct link to VECTOR")

`locked`Booleandefault: false

If true, layer is locked and cannot be edited.

`exportSettings`[ExportSetting](/docs/rest-api/file-property-types/#exportsetting-type)\[\]default: \[\]
...
`preserveRatio`Booleandefault: false
...
`transitionNodeID`String | nulldefault: null

Node ID of node to transition to in prototyping.

`transitionDuration`Number | nulldefault: null
...
`fillGeometry`[Path](/docs/rest-api/file-property-types/#path-type)\[\]default: \[\]

An array of paths representing the object fill. Only present if parameter `geometry=paths` is used.

`fillOverrideTable`Map<Number, [PaintOverride](/docs/rest-api/file-property-types/#paintoverride-type)\>

Map from ID to [PaintOverride](/docs/rest-api/file-property-types/#paintoverride-type) for looking up fill overrides. To see which regions are overriden, you must use the `geometry=paths` option. Each path returned may have an `overrideId` which maps to this table.
...
`individualStrokeWeights`[StrokeWeights](/docs/rest-api/file-property-types/#strokeweights-type)
...
`strokeMiterAngle`Numberdefault: 28.96
...
`strokeGeometry`[Path](/docs/rest-api/file-property-types/#path-type)\[\]
...
Has all the properties of [VECTOR](#vector-props) except `annotations`, plus:
...
### STAR[​](#star-props "Direct link to STAR")

See properties for [VECTOR](#vector-props).

### LINE[​](#line-props "Direct link to LINE")

See properties for [VECTOR](#vector-props).

### ELLIPSE[​](#ellipse-props "Direct link to ELLIPSE")

Has all the properties of [VECTOR](#vector-props), plus:

`arcData`[ArcData](/docs/rest-api/file-property-types/#arcdata-type)

Start and end angles of the ellipse measured clockwise from the x axis, plus the inner radius for donuts.

### REGULAR\_POLYGON[​](#regular-polygon-props "Direct link to REGULAR_POLYGON")

See properties for [VECTOR](#vector-props).

### RECTANGLE[​](#rectangle-props "Direct link to RECTANGLE")
...
### TABLE[​](#table-props "Direct link to TABLE")
...
### TABLE\_CELL[​](#table-cell-props "Direct link to TABLE_CELL")
...
### TEXT[​](#text-props "Direct link to TEXT")

Has all the properties of [VECTOR](#vector-props) except `fillOverrideTable`, plus:

`characters`String

Text contained within a text box.

`style`[TypeStyle](/docs/rest-api/file-property-types/#typestyle-type)
...
`styleOverrideTable`Map<Number, [TypeStyle](/docs/rest-api/file-property-types/#typestyle-type)\>

Map from ID to [TypeStyle](/docs/rest-api/file-property-types/#typestyle-type) for looking up style overrides.

`lineTypes`[LineType](/docs/rest-api/file-property-types/#linetype-type)\[\]
...
### TEXT\_PATH[​](#text-path-props "Direct link to TEXT_PATH")

\[Available in beta, API is subject to change\] Has all the properties of [VECTOR](#vector-props) except `fillOverrideTable`, plus:

`characters`String

Text contained within the text path.

`style`[TextPathTypeStyle](/docs/rest-api/file-property-types/#textpathtypestyle-type)

Style of text including font family and weight (see [TextPathTypeStyle](/docs/rest-api/file-property-types/#textpathtypestyle-type) section for more information).
...
`styleOverrideTable`Map<Number, [TextPathTypeStyle](/docs/rest-api/file-property-types/#textpathtypestyle-type)\>

Map from ID to [TextPathTypeStyle](/docs/rest-api/file-property-types/#textpathtypestyle-type) for looking up style overrides.

`textPathStartData`[TextPathStartData](/docs/rest-api/file-property-types/#textpathstartdata-type)

Data describing the starting position of the text on the path.

### SLICE[​](#slice-props "Direct link to SLICE")
...
### COMPONENT[​](#component-props "Direct link to COMPONENT")

Has all the properties of [FRAME](#frame-props), plus:

`componentPropertyDefinitions`Map<String, [ComponentPropertyDefinition](/docs/rest-api/file-property-types/#componentpropertydefinition-type)\>default: {}

A mapping of name to [ComponentPropertyDefinition](/docs/rest-api/file-property-types/#componentpropertydefinition-type) for every component property on this component. Each property has a `type`, `defaultValue`, and other optional values (see property types section below)

### COMPONENT\_SET[​](#component-set-props "Direct link to COMPONENT_SET")

Has all the properties of [FRAME](#frame-props), plus:

`componentPropertyDefinitions`Map<String, [ComponentPropertyDefinition](/docs/rest-api/file-property-types/#componentpropertydefinition-type)\>default: {}

A mapping of name to [ComponentPropertyDefinition](/docs/rest-api/file-property-types/#componentpropertydefinition-type) for every component property on this component set. Each property has a `type`, `defaultValue`, and other optional values (see property types section below)

### INSTANCE[​](#instance-props "Direct link to INSTANCE")
...
`isExposedInstance`Booleandefault: false

If true, this node has been marked as exposed to its containing component or component set.

`exposedInstances`String\[\]default: \[\]

IDs of instances that have been exposed to this node's level.

`componentProperties`Map<String, [ComponentProperty](/docs/rest-api/file-property-types/#componentproperty-type)\>default: {}

A mapping of name to [ComponentProperty](/docs/rest-api/file-property-types/#componentproperty-type) for all component properties on this instance. Each property has a `type`, `value`, and other optional values (see property types section below)

`overrides`[Overrides](/docs/rest-api/file-property-types/#overrides-type)\[\]default: \[\]

An array of all of the fields directly overridden on this instance. Inherited overrides are not included.

### STICKY[​](#sticky-props "Direct link to STICKY")
...
`authorVisible`Booleandefault: false
...
### SHAPE\_WITH\_TEXT[​](#shape-with-text-props "Direct link to SHAPE_WITH_TEXT")
...
`shapeType`[ShapeType](/docs/rest-api/file-property-types/#shapetype-type)
...
`strokeJoin`Stringdefault: MITER
...
`styles`Map<[StyleType](/docs/rest-api/file-property-types/#style-type), String>

A mapping of a [StyleType](/docs/rest-api/file-property-types/#style-type) to style ID (see [Style](/docs/rest-api/file-property-types/#style-type) of styles present on this node. The style ID can be used to look up more information about the style in the top-level styles field.
...
`connectorStart`[ConnectorEndpoint](/docs/rest-api/file-property-types/#connectorendpoint-type)

Connector starting endpoint.

`connectorEnd`[ConnectorEndpoint](/docs/rest-api/file-property-types/#connectorendpoint-type)

Connector ending endpoint.

`connectorStartStrokeCap`Stringdefault: NONE
...
`connectorEndStrokeCap`Stringdefault: NONE
...
`connectorLineType`[ConnectorLineType](/docs/rest-api/file-property-types/#connectorlinetype-type)
...
`textBackground`[ConnectorTextBackground](/docs/rest-api/file-property-types/#connectortextbackground-type)
...
### WASHI\_TAPE[​](#washi-tape-props "Direct link to WASHI_TAPE")
