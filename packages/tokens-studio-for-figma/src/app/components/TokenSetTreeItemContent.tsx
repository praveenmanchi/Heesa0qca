...
export function TokenSetTreeItemContent({
  ...
  return (
    <TokenSetItem
      ...
      onClick={handleClick}
      isChecked={!item.simplified ? item.checkedState : false}
      item={item}
      onCheck={!item.simplified ? handleCheckedChange : undefined}
      canEdit={!editProhibited && !item.simplified}
      canDuplicate={!isTokensStudioProvider && !tokenSetMetadata[item.path]?.isDynamic && !item.simplified}
      canReorder={!editProhibited && !item.simplified}
      canDelete={item.canDelete && !item.simplified}
