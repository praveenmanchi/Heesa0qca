...
import { ToggleGroup, IconButton, Link } from '@tokens-studio/ui';
...
const Tokens: React.FC<Props> = ({ isActive, simplified }) => {
  ...
  return (
    <TokensContext.Provider value={tokensContextValue}>
      ...
      >
        ...
        >
          {tokenSetsVisible && (
            <TokenSetSelector simplified={simplified} saveScrollPositionSet={saveScrollPositionSet} />
          )}
          <Box
            css={{
              flexGrow: 1,
          ...
          >
            {activeTokensTab === 'json' ? (
              ...
              <Box ref={tokenDiv} css={{ width: '100%', paddingBottom: '$6' }} className="content scroll-container">
                ...
                {!simplified && <ToggleEmptyButton />}
                {showEditForm && !simplified && <EditTokenFormModal resolvedTokens={resolvedTokens} />}
              </Box>
            )}
          </Box>
          {manageThemesModalOpen && <ManageThemesModal />}
        </Box>
        {!simplified && <TokensBottomBar handleError={setError} />}
      </Box>
