...
const TokenGroup: React.FC<React.PropsWithChildren<Props>> = ({
  tokenValues, showNewForm, showForm, schema, path = null, tokenKey, simplified,
}) => {
            ...
            {typeof item.value === 'object' && !isSingleToken(item.value) ? (
              ...
              (
                <StyledTokenGroupItems className="property-wrapper" data-testid={`token-group-${item.stringPath}`}>
                  <TokenGroupHeading showNewForm={showNewForm} label={item.name} path={item.stringPath} id="listing" type={schema.type || tokenKey} simplified={simplified} />
                  <TokenGroup
