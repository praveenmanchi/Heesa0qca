...
export async function sendDocumentChange(event: DocumentChangeEvent) {
    ...
    const variableChanges = event.documentChanges.filter((change) => {
        return change.type === 'PROPERTY_CHANGE' && (change.properties as unknown as string[]).includes('boundVariables');
    });
