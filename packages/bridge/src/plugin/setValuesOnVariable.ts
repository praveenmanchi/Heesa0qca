...
export default async function setValuesOnVariable(
        ...
        try {
          ...
          if (!variable) {
            try {
              ...
              variablesInFigma.push(variable);
              createdVariableNames.push(token.path);
            } catch (e) {
          ...
          if (variable) {
            ...
            if (!createdVariableNames.includes(token.path)) {
              updatedVariableNames.push(token.path);
            }
  ...
  return {
    ...
    details: {
      created: variablesInFigma.filter((v) => !v.remote && !variableKeyMap[v.name]).map((v) => v.name), // This logic needs to be robust
      updated: [], // We need to populate this
      renamed: renamedVariableKeys,
      removed: [], // populated upstream
    },
  };
}
