...
async function getStyleFromNode(node: BaseNode, styleIdBackupKey: string, styleType: 'fills' | 'strokes'): Promise<SelectionStyle[]> {
  const localStyles: SelectionStyle[] = [];

  const localStyle = await getLocalStyle(node, styleIdBackupKey, styleType);
  if (localStyle) {
...
export default async function getAppliedStylesFromNode(node: BaseNode): Promise<SelectionStyle[]> {
  ...
  if ('fills' in node) {
    ...
    localStyles.push(...await getStyleFromNode(node, styleIdBackupKey, 'fills'));
  }

  if ('strokes' in node) {
    const styleIdBackupKey = 'strokeStyleId_original';
    localStyles.push(...await getStyleFromNode(node, styleIdBackupKey, 'strokes'));
  }
