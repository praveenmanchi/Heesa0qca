...
export const attachLocalStylesToTheme: AsyncMessageChannelHandlers[AsyncMessageTypes.ATTACH_LOCAL_STYLES_TO_THEME] = async (msg) => {
  ...
  const paintToKeyMap = await getPaintStylesKeyMap();
  const textStylesToKeyMap = await getTextStylesKeyMap();
  const effectStylesToKeyMap = await getEffectStylesKeyMap();
