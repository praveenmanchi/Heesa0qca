...
type Handler = AsyncMessageChannelHandlers[AsyncMessageTypes.GET_NODE_VARIABLES];
...
export const getNodeVariables: Handler = async (msg) => {
    const nodeId = msg.nodeId;
    const targetNode = await figma.getNodeByIdAsync(nodeId) as SceneNode;
    ...
    const { tree, nodeVarIds } = await buildSelectionTree(targetNode);
