  ...
  if (!checkSetting && selectedThemes && selectedThemes.length > 0) {
    ...
    for (const theme of selectedThemeObjects) {
      ...
      if (collection && modeId) {
        ...
        if (allVariableObj.details) {
          details.created.push(...(allVariableObj.details.created || []));
          details.updated.push(...(allVariableObj.details.updated || []));
          details.renamed.push(...(allVariableObj.details.renamed || []));
          details.removed.push(...(allVariableObj.details.removed || []));
        }
