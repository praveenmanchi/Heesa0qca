...
export async function selectNodes(ids: string[]) {
  const resolved = await Promise.all(ids.map(async (nodeId) => {
    try {
  ...
  const nodes = compact(resolved).filter((node) => (
  ...
  const nodesToSelect = nodes.filter((node) => {
    ...
    return parent && parent.id === currentPage.id;
  });

  currentPage.selection = nodesToSelect;
