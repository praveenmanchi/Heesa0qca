...
export async function trySetStyleId(node: BaseNode, type: StyleType, styleId: string) {
  ...
  try {
    ...
    let actualStyleId: string | null = null;

    // Try resolving a local style by id using async API
    try {
      const localStyle = await figma.getStyleByIdAsync(styleId);
      if (localStyle) {
        actualStyleId = localStyle.id;
      }
    } catch {
      // Fall through to import by key
    }

    // If no local style, try importing by key (for library styles)
    if (!actualStyleId && styleKeyMatch) {
      try {
        const remoteStyle = await figma.importStyleByKeyAsync(styleKeyMatch[1]);
        actualStyleId = remoteStyle.id;
      } catch {
        actualStyleId = null;
      }
    }
