...
export default function useTokens() {
  ...
  const createVariables = useCallback(async () => {
    ...
    if (createVariableResult.details) {
      if (createVariableResult.details) {
        const logs: Omit<LogEntry, 'timestamp'>[] = [];
        createVariableResult.details.created.forEach((name) => logs.push({ type: 'created', name }));
        createVariableResult.details.updated.forEach((name) => logs.push({ type: 'updated', name }));
        createVariableResult.details.renamed.forEach((name) => logs.push({ type: 'renamed', name }));
        createVariableResult.details.removed.forEach((name) => logs.push({ type: 'removed', name }));
        dispatch.changeLogState.addLogs(logs);
      }
    }
  ...
  const updateVariablesFromToken = useCallback(async (payload: UpdateTokenVariablePayload) => {
    ...
    const result = await wrapTransaction({ name: 'updateVariables' }, async () => AsyncMessageChannel.ReactInstance.message({
    ...
    if (result.details) {
      const logs: Omit<LogEntry, 'timestamp'>[] = [];
      result.details.created.forEach((name) => logs.push({ type: 'created', name }));
      result.details.updated.forEach((name) => logs.push({ type: 'updated', name }));
      result.details.renamed.forEach((name) => logs.push({ type: 'renamed', name }));
      result.details.removed.forEach((name) => logs.push({ type: 'removed', name }));
      dispatch.changeLogState.addLogs(logs);
    }
