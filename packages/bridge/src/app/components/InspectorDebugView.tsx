...
export default function InspectorDebugView({ resolvedTokens, _selectedMode }: { resolvedTokens: SingleToken[], _selectedMode?: string }) {
  ...
  function renderBlankslate() {
    ...
    return (
      <EmptyState
        title={uiState.selectedLayers === 1 ? t('noTokensFound') : t('noLayersSelected')}
        description={uiState.selectedLayers === 1 ? 'No tokens are applied to this layer.' : 'Select a layer in Figma to view its applied tokens here.'}
      />
    );
  }
