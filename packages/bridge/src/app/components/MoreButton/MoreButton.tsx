...
export const MoreButton: React.FC<Props> = ({
  ...
  const handleTokenClick = React.useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      if (!simplified && canEdit && ((isMacBrowser && event.metaKey) || (!isMacBrowser && event.ctrlKey))) {
        handleEditClick();
      } else {
        handleClick(properties[0]);
      }
    },
    [canEdit, handleEditClick, handleClick, properties, simplified],
  );
    ...
    <ContextMenu>
      ...
      <ContextMenu.Portal>
        <ContextMenu.Content alignOffset={5} collisionPadding={30}>
          ...
          <ContextMenu.Item onSelect={handleEditClick} disabled={!canEdit || simplified}>
            Edit Token
          </ContextMenu.Item>
          <ContextMenu.Item onSelect={handleDuplicateClick} disabled={!canEdit || simplified}>
            Duplicate Token
          </ContextMenu.Item>
          <ContextMenu.Item onSelect={(event) => handleCopyTokenName(event, token.name)}>
            Copy Token Path
          </ContextMenu.Item>
          <ContextMenu.Item onSelect={handleDeleteClick} disabled={!canEdit || simplified}>
            Delete Token
          </ContextMenu.Item>
        </ContextMenu.Content>
      </ContextMenu.Portal>
