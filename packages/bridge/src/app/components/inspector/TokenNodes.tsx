...
import { selectNodes, goToNodeId } from '@/utils/figma';
...
export default function TokenNodes({ nodes }: { nodes: NodeInfo[] }) {
  const handleSelectComponent = useCallback((componentNodes: NodeInfo[]) => {
    if (!componentNodes.length) return;
    if (componentNodes.length === 1) {
      goToNodeId(componentNodes[0].id);
      return;
    }
    const ids = componentNodes.map((n) => n.id);
    selectNodes(ids);
  }, []);
      ...
      >
        ...
        <>
          {Array.from(componentGroups.entries()).map(([componentName, componentNodes]) => (
            <DropdownMenu.Item
              key={componentName}
              onSelect={() => handleSelectComponent(componentNodes)}
              css={{
                display: 'flex',
                alignItems: 'center',
